...
export const aiAPI = {
  ...
  getUserRecommendations: (limit = 10) =>
    api.get('/recommendations/home', { params: { limit } }),
  getPopularProducts: (limit = 10, category = null) =>
    api.get('/recommendations/home', { params: { limit, category } }),
  getCartRecommendations: (cartItems, limit = 5) =>
    api.get('/recommendations/cart', { params: { limit } }),
  ...
  v2: {
    ...
    trackClick: (recommendationLogId, productId) =>
      api.post('/behavior/track', { eventType: 'recommendation_click', productId }),
    getStatus: () =>
...
export const chatbotAPI = {
  sendMessage: ({ sessionId, message, history = [], context = {} }) =>
    api.post('/v3/chat', { sessionId, message, history, context }),

  /** Tìm kiếm sản phẩm bằng hình ảnh — POST /api/v3/chat/vision */
  sendImage: ({ sessionId, imageBase64, mimeType = 'image/jpeg', history = [], message = '' }) =>
    api.post('/v3/chat/vision', { sessionId, imageBase64, mimeType, history, message })
};
